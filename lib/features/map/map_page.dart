...
class _MapPageState extends State<MapPage> {
  ...
  final LatLng _initialCenter = const LatLng(26.8206, 30.8025);
  ...
  Future<void> loadData() async {
    final data = await MapApiService().getLocations();
  ...
  void applyFilter(String type) {
    setState(() {
      ...
      if (type == "all") {
        ...
        filteredLocations = allLocations.where((e) {
          final t = e.type.toLowerCase().trim();
          return t != "safe" && t != "help";
        }).toList();
      } else {
        filteredLocations = allLocations.where((e) {
          return e.type.toLowerCase().trim() == type;
        }).toList();
      }
    });
  }
  ...
  List<Marker> buildMarkers() {
    return filteredLocations.map((loc) {
      final type = loc.type.toLowerCase().trim();
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
              children: [
                buildButton("Help arrived", "help",
                    const Color(0xFFFFD54F)),
                buildButton("Safe places", "safe",
                    const Color(0xFF81C784)),
                buildButton("Affected areas", "danger",
                    const Color(0xFFDF1111)),
              ],
            ),
          ),
