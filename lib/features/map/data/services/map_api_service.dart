...
class MapApiService {
  ...
  Future<List<MapLocationModel>> getLocations() async {
    try {
      ...
      debugPrint("MAP_DEBUG: starting request, token=$token");
      ...
      debugPrint("MAP_DEBUG: status=${response.statusCode}");
      debugPrint("MAP_DEBUG: body=${response.body}");
      ...
      if (response.statusCode == 200) {
        ...
        // ❌ API error → cache أو fallback
        debugPrint("MAP_DEBUG: API error, trying cache...");
        return await _getCachedPins() ?? _getFallbackData();
      }
    } catch (e) {
      // ❌ No internet أو server down → cache أو fallback
      debugPrint("MAP_DEBUG: exception=$e");
      return await _getCachedPins() ?? _getFallbackData();
    }
  }
}