...
class BluetoothService {
  ...
  void Function(Map<String, String> devices)? onDevicesChanged;
  void Function(SosRequestModel request)? onSosReceived;
  void Function(String sosId, String state)? onSosIdReceived;
  void Function(String endpointId, bool connected)? onConnectionChanged;
  ...
  Future<void> _sendPayload(
      String endpointId, SosRequestModel request) async {
    final json = jsonEncode(request.toBluetoothJson());
    final bytes = utf8.encode(json);
    await _nearby.sendBytesPayload(endpointId, Uint8List.fromList(bytes));
    debugPrint('Payload sent: ${bytes.length} bytes');
  }
  ...
  void _onPayloadReceived(String endpointId, Payload payload) async {
    ...
    try {
      ...
      // لو الـ payload ده assist message من جهاز B لجهاز A
      if (map['type'] == 'assist_coming') {
      ...
      // لو الـ payload ده sosId response من جهاز B لجهاز A
      if (map['type'] == 'sos_id_response') {
        final sosId = map['sosId']?.toString();
        final state = map['state']?.toString() ?? 'pending';
        if (sosId != null) {
          await NotificationService().trackSosId(sosId);
          NotificationService().startPolling();
          onSosIdReceived?.call(sosId, state);
        }
        return;
      }
  ...
  // بعد ما جهاز B يبعت للـ backend → يبعت الـ sosId لجهاز A
  // جهاز B يبعت لجهاز A إن في حد جاي يساعده
  Future<void> sendAssistMessage({
    ...
    try {
      ...
      await _nearby.sendBytesPayload(endpointId, Uint8List.fromList(bytes));
      debugPrint('Assist message sent to: $endpointId');
    } catch (e) {
      debugPrint('sendAssistMessage error: $e');
    }
  }

  Future<void> sendSosIdResponse(String endpointId, String sosId, String state) async {
    try {
      final payload = jsonEncode({
        'type': 'sos_id_response',
        'sosId': sosId,
        'state': state,
      });
      final bytes = utf8.encode(payload);
      await _nearby.sendBytesPayload(endpointId, Uint8List.fromList(bytes));
      debugPrint('SosId response sent to: $endpointId → sosId=$sosId');
    } catch (e) {
      debugPrint('sendSosIdResponse error: $e');
    }
  }
